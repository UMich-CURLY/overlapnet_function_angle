...
namespace cvo{
  ...
  CvoPointCloud::CvoPointCloud(pcl::PointCloud<pcl::PointXYZI>::Ptr pc, int beam_num) {
    ...
    for (int i = 0; i < num_points_ ; i++) {
      ...
      xyz << pc->points[i].x, pc->points[i].y, pc->points[i].z;
      positions_.push_back(xyz);      
      features_(i,0) = pc->points[i].intensity;
    }
    
    std::cout<<"Construct Cvo PointCloud from pcl point cloud with "<<num_points_<<" points"<<std::endl;    
    // write_to_intensity_pcd("range_projection.pcd");
  }
