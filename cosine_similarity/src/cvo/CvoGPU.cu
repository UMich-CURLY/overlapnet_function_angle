...
__global__ void compute_step_size_poly_coeff_location_dependent_ell(float ell,
  ...
  float CvoGPU::indicator_value(const CvoPointCloud& source_points,
    ...
    // float ip_curr = (float)((double)cvo_state.A_host.nonzero_sum / sqrt((double)source_points.num_points() * (double) target_points.num_points()));
    float ip_value = (float)((double)A_sum(&cvo_state.A_host) / sqrt((double)source_points.num_points() * (double) target_points.num_points()));
    std::cout<<"--- indicator_value ---"<<std::endl;
    std::cout<<"nonzeros = "<<(double)cvo_state.A_host.nonzero_sum<<std::endl;
    std::cout<<"nonzero_sum = "<<A_sum(&cvo_state.A_host)<<std::endl;
    ...
    std::cout<<"ip_value = "<<ip_value<<std::endl;
