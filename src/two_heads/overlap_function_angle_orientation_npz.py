...
def overlap_function_angle_orientation_npz(npzfilenames, shuffle=True):
    ...
    for npzfilename in npzfilenames:
        ...
        if len(h.files)==1:
            ...
            function_angle=h[h.files[0]][:,3]
            orientation = h[h.files[0]][:,4]
            n=len(imgf1)
            ...
            function_angle=h['overlaps'][:,3]
            orientation = h['overlaps'][:,4]
            dir1=(h['seq'][:,0]).tolist()
            dir2=(h['seq'][:,1]).tolist()
        ...
        if shuffle:
            ...
            function_angle=function_angle[shuffled_idx]
        ...
        function_angle_all.extend(function_angle)
        orientation_all.extend(orientation)
      
    return (imgf1_all, imgf2_all, dir1_all, dir2_all, np.asarray(overlap_all), np.asarray(function_angle_all), np.asarray(orientation_all))
