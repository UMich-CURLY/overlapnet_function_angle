...
from ImagePairOverlapFunctionAngleSequenceFeatureVolume import ImagePairOverlapFunctionAngleSequenceFeatureVolume
...
(test_imgf1, test_imgf2, test_dir1, test_dir2, test_overlap, test_function_angle, test_orientation) = \
  overlap_function_angle_orientation_npz(testdata_npzfiles, shuffle=False)
...
test_function_angle=test_function_angle[0:no_test_pairs]
...
test_generator_leg=ImagePairOverlapFunctionAngleOrientationSequence(imgpath, test_allimgs, [],
                                                                    [test_dir for _ in range(len(test_allimgs))], [],
                                                                    np.zeros((len(test_allimgs))), np.zeros((len(test_allimgs))), np.zeros((len(test_allimgs))), 
                                                                    network_output_size,
...
test_generator_head=ImagePairOverlapFunctionAngleSequenceFeatureVolume(test_pair_idxs, test_overlap, test_function_angle, 
...
diffs_function_angle = abs(np.squeeze(model_outputs[0])-test_function_angle)
mean_diff_fa = np.mean(diffs_function_angle)
mean_square_error_fa = np.mean(diffs_function_angle*diffs_function_angle)
rms_error_fa = np.sqrt(mean_square_error_fa)
max_error_fa = np.max(diffs_function_angle)
logger.info("  Evaluation on test data results: ")  
logger.info("    Evaluation: mean function angle difference:   %f" % mean_diff_fa)
logger.info("    Evaluation: max  function angle difference:   %f" % max_error_fa)
logger.info("    Evaluation: RMS  function angle error     :   %f" % rms_error_fa)   
