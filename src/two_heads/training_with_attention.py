...
class LossWeightFromAttention(keras.callbacks.Callback):
  """ A class for callback to assign loss_weight after each batch.
  """
  ...
  def on_batch_end(self, batch, logs={}):
    prediction_attention = self.model.get_value('attention_output')
    K.set_value(self.alpha, prediction_attention)
    K.set_value(self.beta, 1 - prediction_attention)
...
# loss weight from attention output
alpha = K.variable(0.5)
beta = K.variable(0.5)
attention_loss_weight = LossWeightFromAttention(alpha, beta)
lossWeights = {"overlap_output": alpha, "orientation_output": beta}
...
for epoch in range(0, no_epochs):
  history = model.fit_generator(train_generator,
                                ...
                                callbacks=[batchLossHistory, learning_rate, attention_loss_weight],
