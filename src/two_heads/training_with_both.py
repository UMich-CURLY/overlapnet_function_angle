...
(train_imgf1, train_imgf2, train_dir1, train_dir2, train_overlap, train_function_angle, train_orientation) = \
  overlap_function_angle_orientation_npz(traindata_npzfiles)
...
if batch_size * no_batches_in_epoch < n:
  ...
  train_function_angle = train_function_angle[0:n]
...
(validation_imgf1, validation_imgf2, validation_dir1, validation_dir2, validation_overlap, validation_function_angle, validation_orientation) = \
  overlap_function_angle_orientation_npz(validationdata_npzfiles, shuffle=False)
if no_test_pairs < validation_overlap.size:
  ...
  validation_function_angle = validation_function_angle[0:no_test_pairs]
...
train_generator = ImagePairOverlapFunctionAngleOrientationSequence(imgpath,
                                                      train_imgf1, train_imgf2, train_dir1, train_dir2,
                                                      train_overlap, train_function_angle, train_orientation, network_output_size,
...
validation_generator = ImagePairOverlapFunctionAngleOrientationSequence(imgpath,
                                          validation_imgf1, validation_imgf2, validation_dir1, validation_dir2, validation_overlap,
                                          validation_function_angle, validation_orientation, network_output_size,
                                          batch_size, inputShape[0], inputShape[1], no_input_channels,
                                          use_depth=use_depth,
                                          use_normals=use_normals,
                                          use_class_probabilities=use_class_probabilities,
...
for epoch in range(0, no_epochs):
  ...
  max_error2 = np.max(diffs2)
  logger.info("  Evaluation on test data results: ")  
  logger.info("           Evaluation: mean function angle difference:   %f" % mean_diff2)
  logger.info("           Evaluation: max  function angle difference:   %f" % max_error2)
