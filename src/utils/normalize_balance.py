...
def normalize_balance(ground_truth_mapping):
  ...
  bin_10_19 = gt_map[(gt_map[:, normalize_idx] < 0.2) & (gt_map[:, 2] >= 0.1)]
  ...
  print('\noverlap raw data distribution\n', distribution)

  normalize_idx = 3
  bin2_0_9 = gt_map[np.where(gt_map[:, normalize_idx] < 0.1)]
  bin2_10_19 = gt_map[(gt_map[:, normalize_idx] < 0.2) & (gt_map[:, 2] >= 0.1)]
  bin2_20_29 = gt_map[(gt_map[:, normalize_idx] < 0.3) & (gt_map[:, normalize_idx] >= 0.2)]
  bin2_30_39 = gt_map[(gt_map[:, normalize_idx] < 0.4) & (gt_map[:, normalize_idx] >= 0.3)]
  bin2_40_49 = gt_map[(gt_map[:, normalize_idx] < 0.5) & (gt_map[:, normalize_idx] >= 0.4)]
  bin2_50_59 = gt_map[(gt_map[:, normalize_idx] < 0.6) & (gt_map[:, normalize_idx] >= 0.5)]
  bin2_60_69 = gt_map[(gt_map[:, normalize_idx] < 0.7) & (gt_map[:, normalize_idx] >= 0.6)]
  bin2_70_79 = gt_map[(gt_map[:, normalize_idx] < 0.8) & (gt_map[:, normalize_idx] >= 0.7)]
  bin2_80_89 = gt_map[(gt_map[:, normalize_idx] < 0.9) & (gt_map[:, normalize_idx] >= 0.8)]
  bin2_90_100 = gt_map[(gt_map[:, normalize_idx] <= 1) & (gt_map[:, normalize_idx] >= 0.9)]

  # print the distribution
  distribution = [len(bin2_0_9), len(bin2_10_19), len(bin2_20_29), len(bin2_30_39), len(bin2_40_49),
                  len(bin2_50_59), len(bin2_60_69), len(bin2_70_79), len(bin2_80_89), len(bin2_90_100)]
  print('\nfunction angle raw data distribution\n', distribution)

  # indexes for overlap choice
  idx_0_9 = np.random.choice(len(bin_0_9), len(bin_40_49)
  idx_10_19 = np.random.choice(len(bin_10_19), len(bin_40_49)
  idx_20_29 = np.random.choice(len(bin_20_29), len(bin_40_49)
  idx_30_39 = np.random.choice(len(bin_30_39), len(bin_40_49)

  idx2_0_9 = np.random.choice(len(bin2_0_9), len(bin2_40_49)
  idx2_10_19 = np.random.choice(len(bin2_10_19), len(bin2_40_49)
  idx2_20_29 = np.random.choice(len(bin2_20_29), len(bin2_40_49)
  idx2_30_39 = np.random.choice(len(bin2_30_39), len(bin2_40_49)

  # concatenate together, take (A or B)
  idx3_0_9 = np.arange(gt_map.shape[0])[np.logical_or(a1, a2)]
...
#   bin_40_49 = bin_40_49[np.random.choice(len(bin_40_49), len(bin_40_49))]
  ...
  print("\nsize of normalized data: ", len(dist_norm_data))
